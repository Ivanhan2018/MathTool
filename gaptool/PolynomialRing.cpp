...
bool PolynomialRing::init(int n,int ID){
	...
	if(pItem){		  
		init(pItem->m_n0,pItem->m_vfvm.c_str());
		return true;		
	}	
	return false;
}

bool PolynomialRing::init(int n,const char* sz){
...
#define PRINT_LOG 0	
	...
	for(int j=i+1;j<r->size();j++)
	{
		...
		if(cnt1>cnt){		
			...
			if(ni<=27 && ID==-1||(ni==8 && ID==12)||(ni==27 && ID==12)||(ni==8 && ID==6)||(ni==27 && ID==9)){					
			//if(ni==16 && std::find(vIDs.begin(),vIDs.end(),ID)!=vIDs.end()){
				printRing0(&S1i,ID);
			}
            if((ni==32||ni==81) && ID>0){
...
int testRingDataA(int argc, char* argv[]){ 
	if(argc<3){
		printf("usage:PolynomialRing n ID|str [g_i] [n0]\n");
		return 0;
	}	
	bool b=false;
